...
- **Save transcript as:** Default is `.html` (compatible with the editor). Optionally `.txt` for plain text or `.vtt` for subtitles and further work in [EXMARaLDA](https://exmaralda.org/). If multiple files are processed at once, only the output folder can be selected. Output file names are then generated automatically.
...
- **Quality:** "Precise" delivers the best results and is the best choice for almost all use cases. "Fast" is a bit quicker and mainly intended for older computers. It requires more post-correction.
- **Mark pauses:** If selected, pauses are marked as dots in brackets, one dot per second of pause. Optional from 1/2/3 seconds.
- **Detect speakers:** If known, select the number of speakers to make detection more robust. Otherwise choose "Auto" for automatic detection or "None" to skip this step entirely.
- **Overlapping speech:** Simultaneous speech is marked with `// Speaker: text of the interjection //` (experimental).
- **Filler words:** If selected, filler words and incomplete words/sentences are transcribed when possible; otherwise they are not. Note: This is more a "recommendation" to the AI model, not a hard on/off switch.
- **Timestamps:** Inserts a timestamp in the format `[hh:mm:ss]` every 60 seconds or on speaker changes (useful for MAXQDA).
...
The editor is an important part of noScribe. It is used to review and correct transcripts. Unfortunately this is still essential even with the best AI models.
...
- Sometimes more speakers are detected than were actually present. Here it helps to set the correct number in "Detect speakers" beforehand.
...
- Like other AI speech models, the Whisper model used here can sometimes "hallucinate" and add words or phrases that sound plausible but were not actually said - see this [Cornell University study](https://facctconference.org/static/papers24/facct24-111.pdf) on the topic.
